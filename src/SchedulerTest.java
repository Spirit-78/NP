...
class Scheduler<T>{
    ...
    T next(){
        Date date = new Date();
        return objects.ceilingEntry(date).getValue();
    }
    T last(){
        Date date = new Date();
        return objects.floorEntry(date).getValue();
    }
    ArrayList<T> getAll(Date begin, Date end){
        return new ArrayList<T>(objects.subMap(begin,end).values());
    }
    T getFirst(){ return objects.firstEntry().getValue();}
    T getLast(){ return objects.lastEntry().getValue();}
}
