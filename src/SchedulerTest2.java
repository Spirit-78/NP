...
class Timestamp<T> implements Comparable<Timestamp<?>>{
    ...
    public int compareTo(Timestamp<?> t){
        if(time.isAfter(t.getTime()))
            return 1;
        else if(time.isBefore(t.getTime()))
            return -1;
        else return 0;
    }
    public boolean equals(Object o){
        ...
        return Objects.equals(time,((Timestamp<?>) o).time);
    }

    @Override
    public String toString() {
        return getTime() + " " + getElement();
    }
}

class Scheduler2<T> extends Timestamp<T>{
    List<Timestamp<T>> list;
    public Scheduler2(){
        super();
        list = new ArrayList<>();
    }
    public Scheduler2(LocalDateTime time, T element) {
        super(time, element);
    }

    public void add(Timestamp<T> t){ list.add(t);}
    public boolean remove(Timestamp<T> t){
        return list.remove(t);
    }
    public Timestamp<T> next(){
        LocalDateTime date = LocalDateTime.now();
        Comparator<Timestamp<T>> com = Comparator.comparing(t ->t.time.compareTo(date));
        return list.stream().sorted(com).filter(t -> t.time.isBefore(date))
...
public class SchedulerTest {
    ...
    public static void main(String[] args) {
        ...
        if (k == 3) { //test Scheduler with String
            Scheduler2<String> scheduler = new Scheduler2<>();
        ...
        if (k == 4) {//test Scheduler with ints complex
            Scheduler2<Integer> scheduler = new Scheduler2<>();
            int counter = 0;
